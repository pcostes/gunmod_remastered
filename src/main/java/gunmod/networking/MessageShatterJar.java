...
public class MessageShatterJar implements IMessage {
	...
	public MessageShatterJar(int particleID, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, int amount) {
	...
	public static class MessageShatterJarHandler implements IMessageHandler<MessageShatterJar, IMessage> {
		// Do note that the default constructor is required, but implicitly defined in
		// this case

		@Override
		public IMessage onMessage(MessageShatterJar message, MessageContext ctx) {
			...
			Minecraft.getMinecraft().addScheduledTask(new Runnable() { 
				...
				public void run() {
					...
					minecraft.spawnParticle(particleType, true, (float) x, (float) y, (float) z, 0, 0, 0);
				}
			});
